# @eventfabric/mediator-opentelemetry
...
- Initial beta release.
  ...
  - **`@eventfabric/mediator`** — abstract `Command` envelope, `CommandBus`, idempotency middleware framework, in-memory `IdempotencyStore`. Future betas extend this with queries and notifications.
  ...
  - **`@eventfabric/sagas`** — `Saga<S, E>` interface (pure reducer over events + timers), `applySagaTransition` runner, `sagaAsAsyncProjection` adapter, `SagaCommandDispatcher`, `SagaTimerScheduler`, in-memory stores, vendor-neutral observer hooks. Sagas emit commands via `@eventfabric/mediator`.
